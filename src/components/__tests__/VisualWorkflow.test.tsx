...
it('restores an article embed and keeps its scope editable', async () => {
  ...
  expect(screen.getByLabelText('Analysis')).toHaveValue('radio');
  expect(document.querySelector('.embed-mode.theme-light')).toBeInTheDocument();
  fireEvent.click(screen.getByText('Adjust session & lap'));
